		...
		for objeto in bucket_s3.objects.all():
			...
			# Some type files will not be analyzed
					...
					## Detecting passwords in the content
						...
						if(fa):
							write_vuln('s3',bucket,key,'VS5','Possible password detected','HIGH - Possible password detected:  BUCKET: ' + bucket + '  FILE: ' + key + '  VARIABLE_NAME: ' + str(fa[0][0]) + '  PASSWORD: ' + str(fa[0][1]), 'HIGH','Low')
						#continue
					## Detecting DNIs
...
for bucket in response_all['Buckets']:
	...
	if(bucket_acl != ''):
		write_vuln('s3',bucket['Name'],'<no_key>','VS3','Bucket with public permissions','MEDIUM - Bucket with public permissions:  BUCKET: ' + bucket['Name'] + '  PERMISSIONS: ' + bucket_acl[:(len(bucket_acl)-1)], 'MEDIUM','High')
		if(args.verbose):
